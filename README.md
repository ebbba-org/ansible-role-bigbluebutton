...
| `bbb_default_presentation_name` | Name of the default presentation file | `default.pdf`| | 
| `bbb_default_presentation` | Location of default presentation | `'${bigbluebutton.web.serverURL}/{{ bbb_default_presentation_name }}'` | |
| `bbb_firewall_ufwbbb_default_presentation_upload` | Allows a standard PDF to be uploaded. | see `defaults/main.yml` |  Can be activated / deactivated. In addition, the local path to the PDF must be specified. The standard PDF will be overwritten unless the name is changed. |
