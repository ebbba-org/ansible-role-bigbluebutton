...
| `bbb_freeswitch_ioschedule_realtime` | [IOSchedulingClass](https://www.freedesktop.org/software/systemd/man/systemd.exec.html#IOSchedulingClass=) | `false` | Set to false for LXD Compatibility, `true` is the default value shipped with freeswtich. Default is false for backward compatibility with the old default (`false`) of this role. |
